        ...
        <Tabs defaultValue="blogs" className="w-full">
          ...
          <TabsContent value="blogs">
            {data.blogs.map((blog) => (
              <Link
                key={blog.id}
                href={`/dashboard/retailers/blogs/${blog.id}`}
              >
          ...
          <TabsContent value="organizations">
            {data.organizations.map((org) => (
              <Link key={org.id} href={`/dashboard/retailers/${org.id}`}>
                <Card className="cursor-pointer hover:shadow-lg transition-all my-1">
                  <CardContent className="p-4">
          ...
          <TabsContent value="products" className="space-y-3">
            {data.products.map((product) => (
              <Link
                key={product.id}
                href={`/dashboard/retailers/products/${product.id}`}
              >
                <Card className="cursor-pointer hover:shadow-lg transition-all my-1">
                  <CardContent className="p-4">
          ...
          <TabsContent value="retailers" className="space-y-3">
            {data.retailers.map((retailer) => (
              <Link
                key={retailer.id}
                href={`/dashboard/retailers/retailers/${retailer.id}`}
              >
